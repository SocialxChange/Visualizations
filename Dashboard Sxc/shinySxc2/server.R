server <- function(input, output) {
    ...
    data1 <- reactive({
            data1 <- filter(data, Nombre_organizacion == "Org 1" & Año == input$periodo)
    })
    ...
    output$grafico5 <- renderPlot({
        ...
        aux <- filter(data, Nombre_organizacion=="Org 1")
        aux <- aux %>%
            group_by(Año) %>%
...
#    })
    ...
    output$texto6 <- renderText({
        x <- paste("Proyectos de la organización:", "Org 1")
        x
    })
    output$texto8 <- renderText({
        x <- paste("Métricas:", "Org 1")
        x
    })
